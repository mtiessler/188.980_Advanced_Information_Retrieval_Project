...
def stream_document_dicts(doc_dir, content_field='fullText'):
    """
    Generator that streams documents from JSONL files in a directory.
    Yields dictionaries with 'id' and combined text fields.
    
    Args:
        doc_dir (str): Directory containing .jsonl files.
        content_field (str): Field to use for main content ('fullText' or 'abstract').
    Yields:
        dict: {'id': doc_id, 'text': combined_text}
    """    
            ...
            with open(filepath, 'r', encoding='utf-8') as f:
                for line in f:
                    try:
                        ...
                        # Validate abstract and main content: more than 10 characters and not a URL
                        abstract = ""
                        ...
                        # Combine fields based on content_field
...
def load_documents_structure(doc_dir, content_field=config.CONTENT_FIELD):
    """
    Loads documents from JSONL files, returning a dictonary of documents and a list of IDs in order.
    
    Args:
        doc_dir (str): Directory containing .jsonl files.
        content_field (str): Field to use for main content (abstract or fullText).
    Returns:
        tuple: (documents dict, list of doc_ids in order)
    """    
            ...
            with open(filepath, 'r', encoding='utf-8') as f:
                for line_num, line in enumerate(f):
                    try:
                        ...
                        # improvement possible: if content_field = abstract -> don't load content_field
                        documents[doc_id] = {
                            'id': doc_id,
                            'title': title,
                            'abstract': abstract,
...
def load_queries(queries_file):
    """
    Loads queries from a tab-separated file.
    
    Args:
        queries_file (str): Path to the queries file.
    Returns:
        dict: Mapping from query_id to query text.
    """    
...
def load_qrels(qrels_file):
    """
    Loads qrels (relevance judgments) from a TREC-formatted file.
    
    Args:
        qrels_file (str): Path to the qrels file.
    Returns:
        list: List of qrel entries.
    """    
